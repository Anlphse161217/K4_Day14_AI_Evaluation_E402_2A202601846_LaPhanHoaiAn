...
class RAGASEvaluator:
    ...
    def evaluate_faithfulness(self, answer: str, context: str) -> float:
        ...
        if not a_tok:
            return 1.0
        return max(0.0, min(1.0, len(a_tok & c_tok) / len(a_tok)))
    ...
    def evaluate_relevance(self, answer: str, question: str) -> float:
        ...
        if not q_tok:
            return 1.0
        return max(0.0, min(1.0, len(a_tok & q_tok) / len(q_tok)))
    ...
    def evaluate_completeness(self, answer: str, expected: str) -> float:
        ...
        if not e_tok:
            return 1.0
        return max(0.0, min(1.0, len(a_tok & e_tok) / len(e_tok)))
    ...
    def evaluate_context_recall(self, contexts: list[str], expected: str) -> float:
        ...
        for c in contexts:
            u_tok |= _tokenize(c)
        return max(0.0, min(1.0, len(e_tok & u_tok) / len(e_tok)))
    ...
    def evaluate_context_precision(
        ...
        rel = 0
        prec_sum = 0.0
        for k, c in enumerate(contexts, 1):
            c_tok = _tokenize(c)
            if len(c_tok & e_tok) / len(e_tok) >= relevance_threshold:
                rel += 1
                prec_sum += rel / k
        return prec_sum / rel if rel > 0 else 0.0
    ...
    def run_full_eval(
        ...
        if contexts is not None:
            res.context_recall = self.evaluate_context_recall(contexts, expected)
            res.context_precision = self.evaluate_context_precision(contexts, expected)
        return res
...
class LLMJudge:
    ...
    def score_response(
        ...
        try:
            ...
            if start != -1 and end != -1:
                ...
                return {"scores": data.get("scores", data), "reasoning": data.get("reasoning", resp)}
        except Exception:
            pass
        return {"scores": {k: 0.5 for k in rubric.keys()}, "reasoning": resp}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        all_s = [v for r in scores_batch for v in r["scores"].values()]
        avg = sum(all_s) / len(all_s) if all_s else 0.5
        pos_bias = False
        if len(scores_batch) > 1:
            first_avg = sum(scores_batch[0]["scores"].values()) / len(scores_batch[0]["scores"])
            rest_avg = sum(sum(r["scores"].values())/len(r["scores"]) for r in scores_batch[1:]) / (len(scores_batch)-1)
            pos_bias = first_avg > rest_avg
        return {
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_context_precision": sum(precs)/len(precs) if precs else None,
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        nr = self.generate_report(new_results)
        br = self.generate_report(baseline_results)
        regs = []
        for k in ['faithfulness', 'relevance', 'completeness']:
            if br.get(f'avg_{k}', 0) - nr.get(f'avg_{k}', 0) > 0.05:
                regs.append(k)
        return {
            "new_avg_faithfulness": nr.get('avg_faithfulness', 0),
            "new_avg_relevance": nr.get('avg_relevance', 0),
            "new_avg_completeness": nr.get('avg_completeness', 0),
            "baseline_avg_faithfulness": br.get('avg_faithfulness', 0),
            "baseline_avg_relevance": br.get('avg_relevance', 0),
            "baseline_avg_completeness": br.get('avg_completeness', 0),
            "regressions": regs,
            "passed": len(regs) == 0
        }
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        min_s = min(f, r, c)
        if sum(1 for x in (f, r, c) if x == min_s) > 1 and min_s < 1.0:
    ...
    def generate_improvement_suggestions(
        ...
        if cats.get("irrelevant", 0) > 0:
            suggs.append("Refine prompt to better capture user intent and question")
        if cats.get("incomplete", 0) > 0:
            suggs.append("Increase chunk size in RAG pipeline to reduce context fragmentation")
        if cats.get("off_topic", 0) > 0:
            suggs.append("Add off-topic guardrails or improve intent classification")
        
        while len(suggs) < 3:
            suggs.append("Further analyze failures for unhandled edge cases")
        return suggs


# ---------------------------------------------------------------------------
# Exercise 3.5 — Bonus: Retrieval Reranking
# ---------------------------------------------------------------------------

def rerank_by_overlap(chunks: list[str], expected: str) -> list[str]:
    """
    Bonus: Rerank retrieved chunks by word overlap with the expected answer.
    """
    import re
    def _tokenize(text: str) -> set[str]:
    ...
    def overlap_score(chunk: str) -> int:
        return len(_tokenize(chunk) & expected_tokens)
